...
class MajorityVoting2(RRAG):
    def query(self, retrieved_docs, prompt, corruption_size, sample_size=3, num_rounds=6, pirac=None, irac=None):
        separate_responses = []
        ...
        if pirac:
            ...
            for _ in range(num_rounds):
                ...
                # LLM에 한 번 요청하고, 결과를 리스트에 저장
                irac_outputs = irac.run(query=prompt , docs=combined_prompt)
